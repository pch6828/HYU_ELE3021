...
int sys_useradd(void){
    ...
    begin_op();
    ilock(userlist);
    int res = useradd(username, password, userlist);
    iupdate(userlist);
    iunlockput(userlist);
    end_op();
    struct inode* ip;
...
int sys_userdel(void){
    ...
    begin_op();
    ilock(userlist);
    int res = userdel(username, userlist);
    iupdate(userlist);
    iunlock(userlist);
    end_op();
    return res;
...
sys_exec2(void)
{
  ...
  if(myproc()->admin == 0 || argstr(0, &path) < 0 || argint(1, (int*)&uargv) < 0 || argint(2, &stacksize) < 0){
    return -1;
  }
  memset(argv, 0, sizeof(argv));
  for(i=0;; i++){
